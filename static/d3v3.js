d3 = function() {
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.point)[0] < 0 ? a[1] - halfÏ€ - Îµ : halfÏ€ - a[1]) - ((b = b.point)[0] < 0 ? b[1] - halfÏ€ - Îµ : halfÏ€ - b[1]);
  }
  ...
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, notHemisphere = Math.abs(cr) > Îµ, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
  ...
  function d3_geo_conicConformal(Ï†0, Ï†1) {
    ...
    function forward(Î», Ï†) {
      var Ï = Math.abs(Math.abs(Ï†) - halfÏ€) < Îµ ? 0 : F / Math.pow(t(Ï†), n);
